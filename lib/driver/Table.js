 ...
 * @returns {String[]}
 */
IndexedDBTable.prototype.__defineGetter__("indexed", function() {
  var tran, store;
  var fields = [];

  //(1) get tran
  tran = this.activeTransaction;
  if (!tran) this.database.native.transaction([this.name], "readonly");

  //(2) get store
  store = tran.objectStore(this.name);

  //(3) get keys
  for (var i = 0, ixs = store.indexNames; i < ixs.length; ++i) {
    fields.push(store.index(ixs[i]).keyPath);
  }
...
/**
 * Checks the query type: by key path (that is, id) or by index.
 *
 * @private
 *
 * @param {String} key  The condition key.
 * @returns {Object}  The key whether by id; the index object whether by index;
 *                    undefined whether invalid.
 */
IndexedDBTable.prototype.checkQueryType = function checkQueryType(key) {
...
IndexedDBTable.prototype.findByFilter = function findByFilter(where, callback) {
  ...
  if (!tran) {
    ...
    tran.oncomplete = function(e) {
      callback(undefined, new Result(records));
    }
  }
...
IndexedDBTable.prototype.findByKeyPath = function findByKeyPath(where, callback) {
  ...
  if (rg) {
    ...
    if (!tran) {
      ...
      tran.oncomplete = function(e) {
        callback(undefined, new IndexedDBResult(records, {byKey: true}));
      };
    }

    //find
    req = tran.objectStore(this.name).openCursor(rg);
...
IndexedDBTable.prototype.findByIndex = function findByIndex(where, callback) {
  ...
  if (rg) {
    ...
    if (!tran) {
      ...
      tran.oncomplete = function(e) {
        callback(undefined, new IndexedDBResult(records, {byIndex: true}));
      };
    }

    //find
    req = tran.objectStore(this.name).index(ix.name).openCursor(rg);
