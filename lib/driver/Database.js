 ...
 * @returns {String[]}
 */
IndexedDBDatabase.prototype.__defineGetter__("objectStoreNames", function() {
  var names = [];

  //(1) get names
  for (var i = 0, stores = this.native.objectStoreNames; i < stores.length; ++i) {
    names.push(String(stores[i]));
  }
...
IndexedDBDatabase.prototype.findTable = function findTable(name, callback) {
  ...
  if (this.containsObjectStore(name)) {
    ...
    tran = this.connection.beginTransaction(undefined, name, {
