...
IndexedDBConnection.prototype.alterDatabase=function(a,b){function c(){var c=f.open(e.config.database);c.onerror=function(a){b&&b(a)};c.onsuccess=function(c){c=c.target.result;var d;d=c.version+1;c.close();c=f.open(e.config.database,d);c.onupgradeneeded=function(b){e.activeTransaction=b.currentTarget.transaction;a(new IndexedDBDatabase(e,b.target.result))};c.onerror=function(a){e.activeTransaction=void 0;b&&b(a)};c.onsuccess=function(a){e.activeTransaction=void 0;a.target.result.close();g?e.open(b):
...
IndexedDBConnection.prototype.hasDatabase=function(a,b){var c=this.indexedDB,d;if(1>arguments.length)throw Error("Callback expected.");1==arguments.length&&(b=arguments[0],a=this.config.database);if("Chrome"==util.getBrowserName())d=this.indexedDB.webkitGetDatabaseNames(),d.onsuccess=function(c){b&&b(void 0,c.target.result.contains(a))},d.onerror=function(a){b&&b(a)};else{var e=!0;d=c.open(a);d.onupgradeneeded=function(a){e=!1};d.onsuccess=function(a){d.result.close();e&&c.deleteDatabase(self);b(void 0,
...
IndexedDBDatabase.prototype.createTable=function(a,b,c){if(1>arguments.length)throw Error("Table name expected.");2==arguments.length&&arguments[1]instanceof Function&&(c=arguments[1],b=void 0);if(this.activeTransaction)if(this.native.objectStoreNames.contains(a))c&&c(Error("Object store '"+a+"' already exists."));else{var d=this.native.createObjectStore(a,b);c&&c(void 0,d)}else c&&c(Error("Database.createTable() only into Connection.createDatabase() or Connection.alterDatabase()."))};
...
IndexedDBDatabase.prototype.hasIndex=function(a,b,c){var d,e;if(3>arguments.length)throw Error("Table name, index name and callback expected.");if(!this.native.objectStoreNames.contains(a))return c(void 0,!1);d=this.activeTransaction;d||(d=this.native.transaction([a],"readonly"),d.onerror=function(a){c(a)},d.oncomplete=function(a){c(void 0,e)});e=d.objectStore(a).indexNames.contains(b);d===this.activeTransaction&&c(void 0,e)};
...
IndexedDBTable.prototype.insert=function(a,b){function c(d){if(d<a.length){var g=e.add(a[d]);g.onerror=function(a){b(a)};g.onsuccess=function(a){c(d+1)}}}var d,e;if(1>arguments.length)throw Error("Object(s) to insert expected.");a?(d=this.activeTransaction,d||(d=this.database.native.transaction([this.name],"readwrite"),d.onerror=function(a){b(a)},d.oncomplete=function(a){b&&b()}),e=d.objectStore(this.name),a instanceof Array||(a=[a]),c(0)):b&&b(Error("Object to insert can't be null or undefined."))};
IndexedDBTable.prototype.save=function(a,b){var c;if(1>arguments.length)throw Error("Object to update expected.");if("object"!=typeof a)throw Error("Object to update expected.");c=this.activeTransaction;c||(c=this.database.native.transaction([this.name],"readwrite"),c.onerror=function(a){b&&b(a)},c.oncomplete=function(a){b&&b()});c.objectStore(this.name).put(a);c===this.activeTransaction&&b&&b()};
